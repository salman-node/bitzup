...
// import { PrismaClient ,buy_sell_order_type} from "@prisma/client";
...
// export const getBuySellBalance = async (req: Request, res: Response) => {
//   try {
...
//     const bal_data: IBuySellBal[] = await prisma.$queryRaw`
//     SELECT 
//     usdt.id as usdt_id,
//     COALESCE(usdt_bal.balance, 0) as usdt_balance,
//     coin.id as coin_id,
//     COALESCE(coin_bal.balance, 0) as coin_balance,
//     coin.qty_decimal as qty_decimal,
//     coin.price_decimal as price_decimal
//   FROM 
//     currencies usdt
//   LEFT JOIN 
//     balances usdt_bal ON usdt.id = usdt_bal.coin_id AND usdt_bal.user_id = ${user_id}
//   LEFT JOIN 
//     currencies coin ON coin.symbol = ${symbol}
//   LEFT JOIN 
//     balances coin_bal ON coin.id = coin_bal.coin_id AND coin_bal.user_id = ${user_id}
//   WHERE 
//     usdt.symbol = 'USDT';
//     `;
...
//       status: "1",
//       data: {
//         usdt_bal: bal_data[0].usdt_balance,
//         coin_bal: bal_data[0].coin_balance,
//         max_amt_limit: bal_data[0].qty_decimal,
//         max_price_limit: bal_data[0].price_decimal,
...
//   try {
//     const { base } = req.body; // remove symbol,order type
...
//     SELECT bs.id,cc.symbol,bs.coin_base,bs.type,bs.usdt_price,bs.quantity,bs.amount,bs.executed_quantity,bs.status,bs.order_type,bs.fees,bs.date_time,bs.cancelled_date_time,bs.order_id 
//     FROM buy_sell_pro_limit_open AS bs
//     JOIN currencies AS cc ON bs.coin_id=cc.id
//     WHERE bs.user_id=${user_id} and bs.status!="OPEN" and bs.coin_base=${base} Order by bs.id DESC;`;
...
//       date_time: formatDate(trade.date_time),
//       cancelled_date_time:
//         trade.cancelled_date_time === 0
//           ? formatDate(new Date().getTime() / 1000)
//           : formatDate(trade.cancelled_date_time),
//       total: Number(trade.executed_quantity) * Number(trade.usdt_price),
//       average: Number(trade.executed_quantity) != 0 ? trade.usdt_price : 0, // get usdt price / total no of filled order from buy sell in  order table else 0
//       amountperIcebergOrder: 0, // in buy sell in order table usdt price will be from call back
//       triggerCondition: "--",
//       sor: "--",
//     }));
...
// export const getAvgPriceOrder = async (req: Request, res: Response) => {
//   const { id: user_id } = req.body.user;
...
//     const avgPrice = await prisma.$queryRaw`
...
//       }
//       const balance: any = currency.main_balance
...
//   const currencyData = await prisma.balances.findFirst({
//     select: {
//       main_balance: true,
//       locked_balance: true
//     },
...
//     const data = {
//       available: currencyData?.main_balance ?? 0.0,
//       unavailable: currencyData?.locked_balance ?? 0.0
//     };
...
// /*----- Get Wallet Funds -------*/
// export const getWalletFunds = async (req: Request, res: Response) => {
//   const { id: user_id } = req.body.user;
...
//     const balanceData: IWalletFunds[] = await prisma.$queryRaw`
...
//     let usdt_bal: number = 0.0;
//     let usdt_coin_id: number | undefined;
//     let coin_balances: { [key: number]: number } = {};

//     balanceData.forEach((entry: BalanceEntry) => {
//       const type: string = entry.type.toUpperCase();
//       const balance: number = parseFloat(entry.balance);
//       const coin_id: number | undefined = entry.coin_id;
...
//     const balData: IWalletAllCurrFunds[] = await prisma.$queryRaw`
...
//     async function useBtcCoinPrice(btc_coin_price: number) {
//       try {
...
//               parseFloat((coin_balances[entry.coin_id] || "0") as string),
//             in_order:
//               parseFloat(entry.balance.toString()) +
//               parseFloat((coin_balances[entry.coin_id] || "0") as string) -
//               entry.balance,
...
//                 (parseFloat(entry.balance.toString()) +
//                   parseFloat(
//                     (coin_balances[entry.coin_id] || "0") as string
//                   ))) /
//               btc_coin_price,
//           }))
//           .sort((a, b) =>
...
//     // use buy sell order
//     const tradeData: ITradeData[] = await prisma.$queryRaw`
...
//           type: trade.type,
//           price: trade.usdt_price,
//           executed: trade.quantity, // how much filled
//           fees: trade.fees,
//           role: "--",
//           sor: "--",
//           realized_pnl: "--",
...
//           totalInUsdt:
//             parseFloat(trade.quantity) * parseFloat(trade.usdt_price),
//           timestamp: trade.date_time,
//         };
//       })
//       .sort((a, b) => parseFloat(b.timestamp) - parseFloat(a.timestamp));
