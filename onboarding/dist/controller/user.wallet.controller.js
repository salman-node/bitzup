...
const getBuySellBalance = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const bal_data = yield prisma.$queryRaw `
    SELECT 
    usdt.id as usdt_id,
...
const getAllBuySellOrder = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    WHERE bs.user_id=${user_id} and bs.status!="OPEN" and bs.coin_base=${base} Order by bs.id DESC;`;
        // Convert each date_time in the buy sell order array
        const convertedTrades = result.map((trade) => (Object.assign(Object.assign({}, trade), { date_time: formatDate(trade.date_time), cancelled_date_time: trade.cancelled_date_time === 0
                ? formatDate(new Date().getTime() / 1000)
                : formatDate(trade.cancelled_date_time), total: Number(trade.executed_quantity) * Number(trade.usdt_price), average: Number(trade.executed_quantity) != 0 ? trade.usdt_price : 0, amountperIcebergOrder: 0, triggerCondition: "--", sor: "--" })));
        // send fetched data to user
        res.status(200).json({
            status: "1",
            data: convertedTrades,
